...
The harness specifies *what* needs to happen (review against specs, preserve rationale) but not *how*. The review-task says "audit against specifications," not "run pytest." If a future model can verify correctness by reading code, no tests needed. If today's model needs tests, it writes and runs them. Hardwiring mechanisms into the harness creates overhead that ages poorly — test suites go stale, linters fall behind, CI configurations drift. The abstraction survives. **Exception: setup.** Bootstrapping requires specific directory creation and .gitignore updates. This is the one mechanistic flow — defining the scaffold that the intent-layer flows operate within.
